        ...
        implements List<E>, RandomAccess, Cloneable, java.io.Serializable
{
    ...
    private class Itr implements Iterator<E> {
        int cursor;       // index of next element to return   指向下一个待返回的元素
        int lastRet = -1; // index of last element returned; -1 if no such
        int expectedModCount = modCount;        /** 记录创建时候的版本 **/
        ...
        public E next() {
            ...
            int i = cursor;
            if (i >= size)      // 比对全局的size，不需要创建时候记录size大小，modCount限制了size不能变化
                throw new NoSuchElementException(); // 超出最大元素数
        ...
        final void checkForComodification() {       /** 比对版本号 **/
            if (modCount != expectedModCount)
                throw new ConcurrentModificationException();
        }
    }
    ...
    static final class ArrayListSpliterator<E> implements Spliterator<E> {
        ...
        private int index; // current index, modified on advance/split      当前坐标,下一个消费的节点
        private int fence; // -1 until used; then one past last index       栅栏      fence初始化为数组大小 size
        ...
        private int getFence() { // initialize fence to size on first use
            ...
            ArrayList<E> lst;
            if ((hi = fence) < 0) { // 没初始化， fence 小于 0
                if ((lst = list) == null)   // 空的
                    hi = fence = 0;
                else {
                    expectedModCount = lst.modCount;    // 初始化时候才会首次初始化 modCount 计数
                    hi = fence = lst.size;              // fence初始化为数组大小 size
                }
            }
            return hi;
        }
        ...
        public ArrayListSpliterator<E> trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1; // 除以2
            return (lo >= mid) ? null : // divide range in half unless too small
                    new ArrayListSpliterator<E>(list, lo, index = mid,      // 拆分一半返回
                            expectedModCount);
        }

        public boolean tryAdvance(Consumer<? super E> action) { /** 消费1个元素 **/
            if (action == null)
